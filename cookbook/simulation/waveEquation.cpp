...
struct MyApp : public App {
  ...
  float decay = 0.96f;      // Decay factor of waves, in (0, 1]
  float velocity = 0.5f;    // Velocity of wave propagation, in (0, 0.5]
  ...
  void onAnimate(double /*dt*/) {
    ...
    for (int j = 0; j < Ny; ++j) {
      for (int i = 0; i < Nx; ++i) {
        ...
        auto val =
            2 * vc - vp + velocity * ((vl - 2 * vc + vr) + (vd - 2 * vc + vu));
