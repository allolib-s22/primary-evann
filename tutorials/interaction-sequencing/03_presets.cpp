...
public:
  void onCreate() override {
    ...
    */
    ...
    /*
        ...
        By default, a PresetServer will listen on OSC path "/preset", although
       this can be changed calling the setAddress() function. The OSC message
       must contain a float or an int value providing the preset index.

        You can also change the morph time by sending a float value to OSC
       address
        "/preset/morphTime".
    */
    presetServer << presetHandler;
    /*
        Adding a listener to a preset server makes the listener receive
       notification of any preset or morph time changes.
    */
    ...
    */
    presetServer.print();
    /* Alternatively, you can use the parameter server for the app to share the
     * port
     */
  ...
  /*
      ...
      also assign a number index to each particular preset. The number index
     will become useful in the next example. For simplicity, the preset name and
     the preset index will be the same (although one is an int and the other a
      string).
  */
